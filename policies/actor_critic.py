...
class ActorCriticPolicy(Policy, BaseModelMixin):
    ...
    def act(self, state, eps=0.1):
        ...
        proba = self.sess.run(self.actor_proba, {self.s: [state]})
        # print(proba)
    ...
    def _build_networks(self):
        ...
        self.actor = tf.squeeze(dense_nn(self.s, self.layer_sizes + [self.act_size], name='actor'))
        print(self.actor.shape)
        self.sampled_actions = tf.squeeze(tf.multinomial(tf.squeeze(self.actor), 1))
        ...
        with tf.variable_scope('actor_train'):
            ...
            self.loss_a = tf.reduce_mean(
                tf.stop_gradient(self.td_error) * tf.nn.sparse_softmax_cross_entropy_with_logits(
                    logits=tf.squeeze(self.actor), labels=self.a), name='loss_actor')  # + 0.001 * self.reg_a
        ...
        for n_episode in range(config.n_episodes):
            ...
            while not done:
                ...
                while buffer.size >= config.batch_size:
                    r_means = np.mean(reward_history[-10:]) if reward_history else 0.0
                    batch = buffer.pop(config.batch_size)
                    _, summ_str = self.sess.run(
                        [self.train_ops, self.merged_summary], feed_dict={
                            self.lr_c: lr_c,
                            self.lr_a: lr_a,
                            self.s: np.expand_dims(batch['s'], 1),
                            self.a: batch['a'],
                            self.r: batch['r'],
                            self.s_next: np.expand_dims(batch['s_next'], 1),
                            self.done: batch['done'],
                            self.ep_reward: r_means,
                        })
